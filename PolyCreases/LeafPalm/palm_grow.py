...
class Curve:
    ...
    def init_lines(self, line_amount):  # 定义curve中的若干条line
        ...
        if self.id == -1:  # 分叉
            ...
            for i in range(line_amount):
                ...
                initial_point = [initial_point[0], initial_point[1] + random_bias]
                Line = TheLine(i, Node(initial_point))  # 初始化Line
                Lines.append(Line)
        return Lines
    ...
    def process_line(self, curve, line):  # 单条line生长
        is_continue = True
...
def show(curve_set, thickness=25):
    ...
    for i in range(secondary_line_num):
        ...
        draw_Secondary_line(image, length, (0, 0, 0), thickness, curve_set)
...
def PalmGrow(feature, ranges, thickness, noise_weight, is_branch=False, is_prolong=False, curve_1_is_cut=False, curve_2_is_cut=False):
    ...
    curve_1 = Curve(1, curve_param_1, curve_range_1, grow_direction=0, noise_weight=noise_weight, is_cut=curve_1_is_cut)
    curve_2 = Curve(2, curve_param_2, curve_range_2, grow_direction=1, noise_weight=noise_weight, exist_branch=is_branch, is_prolong=is_prolong,
                    is_cut=curve_2_is_cut)
    curve_3 = Curve(3, curve_param_3, curve_range_3, grow_direction=1, noise_weight=noise_weight)
    ...
    # 显示
    image = show(curve_set, thickness)
    return image
